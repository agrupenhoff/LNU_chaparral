...
str(propnativeRich)
propnativeRich$num_burn <- as.numeric(propnativeRich$num_burn)
...
# Plug a dataset where quota is FALSE and TRUE into the model
beta_bayes_pred <- m.propnatRich2 %>% 
...
# Plug a dataset where quota is FALSE and TRUE into the model
propnativeRich_small %>% 
  data_grid(num_burn=seq(1,7,by=.1),
            year=c(2021, 2022)) %>% 
  add_draws(posterior_epred(m.propnatRich2, newdata= .)) %>% 
  ggplot(aes(x=num_burn, y=prop_native_rich))+
  stat_lineribbon(aes(y=.value),alpha=.35,.width = c(.66, .95))+
  geom_point(data=propnativeRich_small , alpha=.2)+
  geom_jitter(data=propnativeRich_small, alpha =.2)+
  facet_wrap(~year)+
...
Model comparisons:
                        elpd
m.prop.cover.rich_num Burn  19.9 
m.prop.cover.rich_year   13.3
m.prop.cover.rich_hli   9.7
HLI values range from 0 (coolest) to 1 (hottest).
m.prop.cover.rich_ppt   6.4
m.prop.cover.rich_tmean 6
m.prop.cover.rich_slope 5.4
m.prop.cover.rich_TSLF. 26.8
...
plot(conditional_effects(m.prop.cover.rich3, points=T))
