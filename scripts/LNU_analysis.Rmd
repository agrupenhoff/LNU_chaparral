...
loo(m.propnatRich,m.propnatRich2b,  m.propnatRich3, m.propnatRich4)
...
ggplot(beta_bayes_pred, aes(x = num_burn, y = .epred, fill = ppt)) +
  stat_halfeye(.width = c(0.8, 0.95), point_interval = "median_hdi") +
  scale_fill_manual(values = cal_palette("sierra1")) +
  labs(x = "Predicted proportion of native species", y = NULL,
       caption = "80% and 95% credible intervals shown in black")
...
str(propnativeRich_small)
propnativeRich_small$ppt <- as.numeric(propnativeRich_small$ppt)
#run random forest model
ctree1 <- ctree(prop_native_rich ~ num_burn + year + hli + cool_warm_slope + ppt,
                    data = propnativeRich_small,
...
rpartv1 <- rpart(prop_native_rich ~ num_burn + year + hli + cool_warm_slope, ppt,
                    data = propnativeRich_small,
                    method = "class", 
                    control = rpart.control(cp=.2))
rpart.plot(rpartv1, box.palette = "blue")
summary(rpartv1)
#prediction of fitted mean and visualization
plot(prop_native_rich ~., data = propnativeRich_small)
