...
ggplot(LNU_cover_prop, aes(x=as.numeric(num_burn), y=cover))+
  geom_smooth()
...
ggplot(beta_bayes_pred %>% filter(num_burn != "7"), 
       aes(x = num_burn, y = .epred, fill=num_burn)) +
  #stat_pointinterval(position = position_dodge(width = .4),
  #                               .width = c(0.95))+
 stat_halfeye(.width = c(0.8, 0.95), point_interval = "median_hdi") +
  scale_fill_manual(values = cal_palette("sierra1")) +
  labs(y = "Predicted proportion of native species (cover)", 
       x = "Fire Frequency",
       caption = "80% and 95% credible intervals shown in black")
...
hist(shrub_density_OBLS$avg.count)
hist(shrub_density_OBLS$prop.presence)
plot(shrub_density_OBLS$num_burn, shrub_density_OBLS$prop.presence)
...
m.shrubDensityOBL2 <- brm(data = shrub_density_OBLS,
                        bf(prop.presence ~ num_burn + year,
                           phi ~ num_burn + year,
                           zi ~ num_burn + year),
                      family = zero_inflated_beta(),
                      #control = list(adapt_delta = 0.998, max_treedepth = 15),
...
m.shrubDensityOBL3 <- brm(data = shrub_density_OBLS,
                        bf(prop.presence ~ num_burn + year + ppt + tmean + hli,
                           phi ~ num_burn + year,
                           zi ~ num_burn),
                      family = zero_inflated_beta(),
                      #control = list(adapt_delta = 0.998, max_treedepth = 15),
...
ctree1 <- ctree(presence ~ num_burn + elevation + aspect_name,
                    data = OBLS_den_short,
                    control = ctree_control(testtype = "Bonferroni"))
ctree1
plot(ctree1)
